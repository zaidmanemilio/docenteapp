...
// Refactor: el modal de edición fue extraído a src/components/schedule/SessionModal.tsx
// Sin cambios de comportamiento ni diseño.
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
      ...
      {bulkMode && (
        <div style={{
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <select
              onChange={e => e.target.value && bulkChangeStatus(e.target.value)}
              defaultValue=""
            ...
            <select
              onChange={e => e.target.value && bulkChangeModality(e.target.value)}
              defaultValue=""
              disabled={!selected.size || bulkSaving}
              style={{ padding: '5px 8px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', border: '1px solid #4338ca', background: '#312e81', color: '#c7d2fe', fontFamily: 'inherit' }}
            >
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        {/* Tipo — solo muestra los que existen en este curso */}
        ...
        {/* Comisión, Modalidad, Responsable */}
                  ...
                  <tr key={s.id} style={{ borderBottom: '1px solid #f3f4f6', background: selected.has(s.id as string) ? '#eef2ff' : 'white' }}>
                    {bulkMode && (
                      <td style={{ padding: '10px 12px' }}>
                        <input
                          type="checkbox"
                          checked={selected.has(s.id as string)}
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <Badge text={SESSION_TYPE_LABELS[s.type] || s.type} style={TYPE_BADGE[s.type]} />
                    </td>
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <Badge text={SESSION_STATUS_LABELS[s.status] || s.status} style={STATUS_BADGE[s.status]} />
                    </td>
                    <td style={{ padding: '10px 12px' }}>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        {LINK_FIELDS.map(field => {
                          ...
                          return url ? (
                            ...
                            <span key={field}
                              style={{ width: '24px', height: '24px', borderRadius: '4px', background: '#f3f4f6', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#d1d5db', fontSize: '13px' }}>
                              <i className={`ti ${LINK_ICONS[field]}`} aria-hidden="true"></i>
                            </span>
                          )
                        })}
